...
def generate_screenshots(objects, fov, filename, times=None, html_slow_down=1,
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(args = chrome_args)
        headless = k3d_remote(pl, browser, port=find_free_port())
        headless.sync()
        pl.camera_fov = fov
...
def write_animation(img_dir, filename, total_duration):
    ...
    img = []
    cbar_path = f"{img_dir}/colorbar.png"
    cbar = Image.open(cbar_path)
    add_h = 100
    for f in os.listdir(img_dir):
        if f.endswith(".png") and "colorbar" not in f:
            imgi = Image.open(f"{img_dir}/{f}")
            w,h = imgi.size
            imgi = imgi.crop((200, 0, w - 200, h))
            w,h = imgi.size
            dst = Image.new('RGB', (w, h + add_h))
            dst.paste(imgi, (0, 0))
            dst.paste(cbar, (int(w/2 - cbar.size[0]/2), h + add_h - cbar.size[1]))
            img.append(dst)
    os.makedirs(os.path.dirname(filename), exist_ok=True)
