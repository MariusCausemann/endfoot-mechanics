...
ax_pvs_p = results["pvs_axial_pore_pressure"]

plot_times = np.linspace(T - 1/f, T, 6)
plot_idx = [np.argmin(abs(times - pt)) for pt in plot_times]
plt.figure()
for i in plot_idx:
    plt.plot(y_coords, ax_pvs_p[:,i], label=f"t={np.round(times[i],2):.2f}s")
plt.xlabel("y [\u03BCm]")
plt.ylabel("pressure [Pa]")
plt.legend()
if __name__ == '__main__':
    plt.savefig(f"results/{mesh_name}_{sim_name}/plots/{mesh_name}_{sim_name}_axial__pore_pressure.png")
